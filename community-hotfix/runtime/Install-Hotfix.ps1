...
try {
    ...
    if (Test-RecordedFileSet -Root $environment.WorkshopRoot -Files $sourceFiles) {
        if (Test-Path -LiteralPath $backupRoot -PathType Container) {
            if (-not (Test-RecordedFileSet -Root $backupRoot -Files $sourceFiles)) {
                Write-Host 'Rebuilding the incomplete rollback backup...'
        ...
        if (-not $currentEdition -and $manifest.PSObject.Properties['legacyPatchedWorkshopFiles']) {
        ...
        if (-not $currentEdition -and -not $matchedLegacyPatch) {
            throw 'The Workshop VPK files do not match the supported original files or this hotfix. Run Diagnostics.cmd before changing anything.'
        }
        if (-not (Test-RecordedFileSet -Root $backupRoot -Files $sourceFiles)) {
            throw "The map is already patched, but its verified rollback backup is missing: $backupRoot"
        }
        $alreadyApplied = ($currentEdition -eq $Edition)
        if ($matchedLegacyPatch) {
            Write-Host 'Recognized a supported previous patch; it will be replaced safely.'
        }
    }
    ...
    if ($alreadyApplied) {
        ...
        Write-Host ("Building the {0} Workshop patch from the verified rollback backup..." -f $Edition)
    ...
    Write-Host 'The patch stays installed across normal launches. Launch.cmd only reapplies it if Steam restores the supported original files.'
}
catch {
    Write-Host ''
